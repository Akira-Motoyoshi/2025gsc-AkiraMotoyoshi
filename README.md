...
セクションが短くなりすぎると GPS 誤差や計測誤差の影響が相対的に大きくなるため、全ての交差点でセクションを分けず、セクションの数は限界まで減らす。
![section_image.png](section_map.png)
...
その結果、導き出された最短ルートは以下の通りである。
level1: A1,A2　289 sec
level2: E2 250 sec
...
信号待ち時間や横断回数を考慮すると、距離がやや長いルートであっても、  
停止回数の少ないルートの方が総所要時間が短くなるケースが確認された。毎日通り、安定して最速を求めるためには長い信号待ちを避けることが非常に重要であることがわかった。  
また、街灯密度の分析から、夜間においては最短距離ルートが必ずしも安全とは言えない可能性が示された。
![導き出された最短ルート](<img width="1440" height="900" alt="スクリーンショット 2026-02-01 16 47 54" src="https://github.com/user-attachments/assets/47d05fbe-f0a5-4b8b-86a0-0b327ebcaac4" />
)
---
...
信号待ちや横断による遅延は、日常的な徒歩移動において無視できない影響を及ぼす。地図アプリで表示された数字上の最短も、障害物などを考慮した実測から導き出される最速に負けることがある。  
